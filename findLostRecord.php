<?php
  ...
  if($handle = opendir($dataBaseDir)) 
  {
    while($entry = readdir($handle)) 
    {
      if(is_dir($dataBaseDir."/".$entry) && $entry!=='.' && $entry!=='..') 
      {
        ...
        // Если директория не отмечена в XML-файле и считается потерянной при синхронизации
        if( $result!==true )
        {
  ...
  // Печать результата для незашифрованных данных
  foreach($noCryptRecord as $recordDirName)
  {
    echo '<record id="'.getUnicalId().'" dir="'.$recordDirName.'" name="Потерянная запись в директории '.$recordDirName.'" file="text.html" ctime="20101114163214"/>'."\n";
  }
